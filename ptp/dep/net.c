...
bool netShutdown_UDP(NetPath *netPath)
{
...
bool netInit_UDP(NetPath *netPath, PtpClock *ptpClock)
{
...
static ssize_t netRecv_UDP(octet_t *buf, TimeInternal *time, BufQueue *msgQueue)
{
	int i;
	int j;
	u16_t length;
...
static ssize_t netSend_UDP(const octet_t *buf, int16_t  length, TimeInternal *time, const int32_t * addr, struct udp_pcb * pcb)
{
	err_t result;
	struct pbuf * p;
...
ssize_t netSendPeerGeneral(NetPath *netPath, const octet_t *buf, int16_t  length)
{
	...
	netSendL2(netPath, buf, length, NULL);
}
...
ssize_t netSendPeerEvent(NetPath *netPath, const octet_t *buf, int16_t  length, TimeInternal* time)
{
	...
	netSendL2(netPath, buf, length, time);
}
