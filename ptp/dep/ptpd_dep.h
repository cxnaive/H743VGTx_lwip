...
#define PTPD_DBG
#define PTPD_ERR
#define DBGV(...)  { TimeInternal tmpTime; getTime(&tmpTime); usb_printf("(d %d.%09d) ", tmpTime.seconds, tmpTime.nanoseconds); usb_printf(__VA_ARGS__); }
#else
#define DBGV(...)
#endif
...
#ifdef PTPD_DBG
#define PTPD_ERR
#define DBG(...)  { TimeInternal tmpTime; getTime(&tmpTime); usb_printf("(D %d.%09d) ", tmpTime.seconds, tmpTime.nanoseconds); usb_printf(__VA_ARGS__); }
#else
#define DBG(...)
#endif
/** \}*/
...
/**\{*/
#ifdef PTPD_ERR
#define ERROR(...)  { TimeInternal tmpTime; getTime(&tmpTime); usb_printf("(E %d.%09d) ", tmpTime.seconds, tmpTime.nanoseconds); usb_printf(__VA_ARGS__); }
...
void netQInit(BufQueue *queue);
bool netQPut(BufQueue *queue, void *pbuf);
void *netQGet(BufQueue *queue);
void netQEmpty(BufQueue *queue);
bool netQCheck(BufQueue  *queue);
ssize_t netRecv(octet_t *buf, TimeInternal *time, BufQueue *msgQueue);
ssize_t netSend(const octet_t *buf, int16_t  length, TimeInternal *time, const int32_t * addr, struct udp_pcb * pcb);
